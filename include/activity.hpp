...
public:
  ...
  Activity(Info infoStruct) : _id(infoStruct.id.c_str()), _sport(infoStruct.sport.c_str())
  {
    loadInfo(infoStruct);
  }
  ...
  const char *getSportType() const
  {
    return _act->attribute("Sport").value();
  }

  const char *getId() const {
    return _act->child("Id").first_child().value();
  }
